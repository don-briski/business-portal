...
@Component({
  selector: "lnd-acc-activity-cipher",
  templateUrl: "./acc-activity-cipher.component.html",
  styleUrls: ["./acc-activity-cipher.component.scss"],
})
export class AccActivityCipherComponent implements OnInit, OnDestroy {
  @Input() config: AccActivityCipherReqBody;
  @Input() user: User;
  ...
  form = new FormGroup({
    minInflowsCount: new FormControl(null, [
      Validators.required,
      Validators.min(1),
    ]),
    passRate: new FormControl(null, [
      Validators.required,
      Validators.min(0),
      Validators.max(100),
    ]),
  ...
  submit() {
    ...
    this.checkoutAdminService
      .configureAccActivity(this.form.value as AccActivityCipherReqBody)
      .pipe(takeUntil(this.subs$))
      .subscribe(
        (res) => {
          ...
          this.toast.fire({
            type: "success",
            title: "Account activity cipher updated successfully!",
          });
