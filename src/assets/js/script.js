  ...
  hoverCard: {
    ...
    showThumb: function () {
      ...
      if (bodyWidth > 767) {
        ...
        var name = this.handleRef.data("name")
          ? this.handleRef.data("name")
          : "";

        var innerHtml =
          '<span class="user-bg-card__info"><span class="dt-avatar-name text-center">' +
          name +
          "</span></span>";
...
(function ($) {
  ...
  if ($loader.length) {
    $loader.delay(300).fadeOut("normal", function () {
      $root.css("opacity", "1");
  ...
  $(function () {
    $(".styled").click(function () {
      $(this).text(function (i, text) {
        return text === "Show advanced options"
  ...
  // get table checkbox count
  var $checkboxes = $('.table-responsive td input[type="checkbox"]');
  $checkboxes.change(function () {
    var countCheckedCheckboxes = $checkboxes.filter(":checked").length;
    $(".count-checked-checkboxes").text(countCheckedCheckboxes);
  });

  // get List checkbox count
  var $listcheckboxes = $(
    '.module_new .dt-module__content-inner .dt-module__list .dt-module__list-item input[type="checkbox"]'
  );
  $listcheckboxes.change(function () {
    var countListCheckedCheckboxes = $listcheckboxes.filter(":checked").length;
    $(".count-list-checked-checkboxes").text(countListCheckedCheckboxes);
    $(".claim_button").show();
  });

  var $listcheckboxes1 = $('.module_new input[type="checkbox"]');
  $listcheckboxes1.change(function () {
    var countListCheckedCheckboxes1 =
      $listcheckboxes1.filter(":checked").length;
    $(".count-list-checked-checkboxes1").text(countListCheckedCheckboxes1);
    $(".claim_button").show();
  });

  // click tab on page load.
  $(document).ready(function () {
    $(".clicked").click();
  });

  /* customizer settings */
  var $dtTheme = sessionStorage.getItem("dt-theme");
  var $dtLayout = sessionStorage.getItem("dt-layout");
  var $dtStyle = sessionStorage.getItem("dt-style");
  ...
  $("#theme-style-chooser")
    .find("[data-style=" + $currentThemeStyle + "]")
    .addClass("active");
  $("#layout-chooser")
    .find("[data-layout=" + $currentLayout + "]")
    .parent()
    .addClass("active");
      ...
      function () {
        ...
        if (bodyWidth > 767) {
          ...
          if (userPic) {
            ...
            $(".profile-placeholder", $userInfoCard)
  ...
  if ($("#user-menu-dropdown").length) {
    var userDorpdown = $("#user-menu-dropdown")
      .find(".dropdown-menu")
      .clone(true)
      .appendTo("body");
...
function handleSidbarMenu() {
  ...
  if (jQuery("body").hasClass("dt-sidebar--folded")) {
    if (!hoverEventExcuted) {
      ...
      jQuery(".dt-sidebar--folded ul.dt-side-nav > li.dt-side-nav__item").each(
        function () {
          var $currentMenuItem = jQuery(this);
          var $submenu = jQuery(".dt-side-nav__sub-menu", $currentMenuItem);
          var ps;

          $currentMenuItem.find("> a.dt-side-nav__link", $submenu).hover(
            function () {
              var $menuLink = jQuery(this);

              if (
                $submenu.length &&
                jQuery("body").hasClass("dt-sidebar--folded")
              ) {
                $submenu.hover(
                  function () {
                  ...
                  function () {
                    if (ps) ps.destroy();
                    $submenu
                      .removeClass("active dt-side-nav__balloon")
                      .attr("style", "");
                  }
                );
                ...
                var offsetTop =
                  positionValue.top +
                  (menuLinkHeight - $menuLinkTextHeight) / 2;
              ...
              if ($submenu.length) {
                setTimeout(function () {
                  if (!$submenu.hasClass("active")) {
                    if (ps) ps.destroy();
                    $submenu
                      .removeClass("dt-side-nav__balloon")
                      .attr("style", "");
                  }
                }, 300);
              } else {
    ...
    if (!clickEventExcuted) {
      ...
      jQuery("ul.dt-side-nav > li.dt-side-nav__item").on("click", function () {
        var menuLi = this;
        jQuery("ul.dt-side-nav > li.dt-side-nav__item")
          .not(menuLi)
          .removeClass("open");
        jQuery("ul.dt-side-nav > li.dt-side-nav__item ul")
          .not(jQuery("ul", menuLi))
      ...
      jQuery("ul.dt-side-nav__sub-menu li").on("click", function (e) {
        ...
        if ($current_sm_li_parent.parent().hasClass("active")) {
          jQuery("li ul", $current_sm_li_parent)
            .not(jQuery("ul", $current_sm_li))
            .slideUp(slideDuration, function () {
              jQuery("li", $current_sm_li_parent)
                .not($current_sm_li)
                .removeClass("active");
            });
        } else {
          jQuery("ul.dt-side-nav__sub-menu li ul")
            .not(jQuery(" ul", $current_sm_li))
            .slideUp(slideDuration, function () {});
        }
...
function changeLayout($layout, $init) {
  ...
  if ($layout === "framed") {
    $body
      .removeClass("dt-layout--boxed dt-layout--full-width")
      .addClass("dt-layout--framed");
  } else if ($layout === "full-width") {
    $body
      .removeClass("dt-layout--framed dt-layout--boxed")
      .addClass("dt-layout--full-width");
  } else if ($layout === "boxed") {
    $body
      .removeClass("dt-layout--framed dt-layout--full-width")
      .addClass("dt-layout--boxed");
  }

  sessionStorage.setItem("dt-layout", $layout);
  $(document).trigger("layout-changed");

  if (!$init) notifyUser("Layout Updated successfully.");
}
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
  ...
  if ($theme === "lite") {
    ...
    // $logo.attr('src', 'assets/images/logo-blue.png');¿
    ...
    // $body.find('.dt-header .dt-brand__logo-img').attr('src', 'assets/images/logo-blue.png');
  ...
  sessionStorage.setItem("dt-theme", $theme);
  sessionStorage.setItem("dt-style", $style);
  $(document).trigger("theme-changed");

  if (!$init) notifyUser("Theme Updated successfully.");
}
...
function init_indecator() {
  ...
  $progressbar.each(function (index) {
    ...
    var maxVal = $currentBar.data("max")
      ? parseInt($currentBar.data("max"))
      : 100;
    var fillTypePercent = $currentBar.data("percent") ? true : false;
    var textSufix = $currentBar.data("suffix")
      ? $currentBar.data("suffix")
      : "";
...
var claasName = "pool-nav";
var noElementHasClass = !document.getElementsByClassName(claasName).length;

if (!noElementHasClass) {
  var btns = header.getElementsByClassName(claasName);
